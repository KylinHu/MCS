% spacial position for number at generation location
%%% intesity,duration in each pixel during whole track time
clear 
clc
...
%% 
% num2 =zeros(181,1080); 
init_num =zeros(1080,181,240); 
cent_num =zeros(1080,181,240); 
...
%     num1 =zeros(181,1080); 
    ...
    for i = 1:length(mcs_track)
        track = mcs_track(i);
        xc = round(mcs_track(i).list_shield_x(1));
        yc = round(mcs_track(i).list_shield_y(1));
        cent_num(yc,xc,monnum) = cent_num(yc,xc,monnum)+1;
%        for t = 1:track.age
%            pixelnum = length(mcs_track(i).list_pixel{t});
            pixelnum = size(mcs_track(i).list_pixel{1},1);
            for n = 1:pixelnum
                %fprintf('debug %d %d %d \n',n, i, pixelnum);
                x = mcs_track(i).list_pixel{1}(n,1);
                y = mcs_track(i).list_pixel{1}(n,2);
%                 num1(x,y) = num1(x,y)+1;
%                 num2(x,y) = num2(x,y)+1;
                init_num(y,x,monnum) = init_num(y,x,monnum)+1;
...
init_num = init_num(:,:,index);
cent_num = cent_num(:,:,index);
%tem = tem./num;
%duration  = duration./num;
%coverage = coverage./num;
file_name = strcat('num_init_','yr86yr05monthly_exp7','.mat');
savefile = fullfile(output_path, file_name);
save(savefile,'init_num','cent_num','time')
