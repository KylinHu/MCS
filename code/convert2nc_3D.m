...
input_name = 'pixel_yr86yr05monthly_exp6.mat';   %alltime series
load (fullfile(input_path, input_name));
% added for sort time series 
...
ncid = netcdf.create('../data/pixel_yr86yr05monthly_exp6.nc','NC_CLOBBER'); 
...
%     netcdf.endDef(ncid);
    ...
    netcdf.putAtt(ncid,varid,'units','n');                                                     %?????long_name???????????
    netcdf.putAtt(ncid,varid1,'units','K');   
    netcdf.putAtt(ncid,varid11,'units','hours');                                                     %?????long_name???????????
    ...
    netcdf.putAtt(ncid,varid,'long_name','The MCS number');
